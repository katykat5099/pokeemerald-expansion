...
void GiveBoxMonInitialMoveset(struct BoxPokemon *boxMon)
{
    ...
    for (i = 0; learnset[i].move != LEVEL_UP_MOVE_END; i++)
    {
        ...
        {
            move = GetRandomMove(learnset[i].move, species);
...
u8 CalculatePartyCount(struct Pokemon *party)
{
    ...
    while (partyCount < GetMaxPartySize() //tx_randomizer_and_challenges
...
u8 GetTypeBySpecies(u16 species, u8 typeNum)
{
    ...
    #ifndef NDEBUG
    if (gSaveBlock1Ptr->tx_Random_Type)
        MgbaPrintf(MGBA_LOG_DEBUG, "TX RANDOM TYPE%d: species=%d=%S; type=%d=%S", typeNum, species, gSpeciesInfo[species].speciesName, type, gTypeNames[type]);
    #endif
...
static u16 GetRandomSpecies(u16 species, u8 mapBased, u8 type, u16 additionalOffset) //INTERNAL use only!
{
    ...
    if (gSaveBlock1Ptr->tx_Random_Similar)
    {
        ...
        case EVO_TYPE_2:
            ...
            break;
        case EVO_TYPE_NONE:
            speciesResult = sRandomSpeciesEvoNone[RandomSeededModulo(species + mapOffset + additionalOffset, RANDOM_SPECIES_EVO_NONE_COUNT)];
            break;
        ...
        MgbaPrintf(MGBA_LOG_DEBUG, "%S: species=%d=%S; mapBased=%d; speciesResult=%d=%S; %S-->>%S", gRandomizationTypes[type], species, gSpeciesInfo[species].speciesName, mapBased, speciesResult, gSpeciesInfo[speciesResult].speciesName, gEvoStages[slot], gEvoStages[slotNew]);
        #endif
...
/*FURFROU*/   if((GET_BASE_SPECIES_ID(GetMonData(&gPlayerParty[i], MON_DATA_SPECIES)) == SPECIES_FURFROU) && (*var == 676))
                { return TRUE; }
/*HOOPA*/     if((GET_BASE_SPECIES_ID(GetMonData(&gPlayerParty[i], MON_DATA_SPECIES)) == SPECIES_HOOPA) && (*var == 720))
                { return TRUE; }
/*ORICORIO*/  if((GET_BASE_SPECIES_ID(GetMonData(&gPlayerParty[i], MON_DATA_SPECIES)) == SPECIES_ORICORIO) && (*var == 741))
                { return TRUE; }
